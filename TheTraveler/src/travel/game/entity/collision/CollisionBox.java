...
public class CollisionBox {
	...
	private boolean touchTop(Entity entity) {
		if ((entity.getY() + entity.getHeight()) < (mainEntity.getY() + pushBackDistance)) {
			return true;
		}
	...
	private boolean touchBottom(Entity entity) {
		if (entity.getY() > (mainEntity.getY() + mainEntity.getHeight() - pushBackDistance)) {
			return true;
		}
	...
	private boolean touchLeft(Entity entity) {
		if ((entity.getX() + entity.getWidth()) < (mainEntity.getX() + pushBackDistance)) {
			return true;
		}
	...
	private boolean touchRight(Entity entity) {
		if (entity.getX() > (mainEntity.getX() + mainEntity.getWidth() - pushBackDistance)) {
			return true;
		}
	...
	public void suckInAndShrink(Entity entity) {
		...
		int ourMidY = (mainEntity.getY() + mainEntity.getHeight()) / 2;
