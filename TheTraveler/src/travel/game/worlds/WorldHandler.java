...
public class WorldHandler {
	...
	public static void loadWorld(Worlds world) {
		...
		sizeOfWorld = new Dimension(input.getWidth(), input.getHeight());
	...
	private static void pickWorldTexture(Worlds world) {
		switch (world) {
		case world1:
			background = ImageHandler.getImage("res/worlds/textures/world1.png");
			break;
		}
	}
	
	private static void addBackground() {
		entities.addEntity(new Background(0, 0, (int)sizeOfWorld.getWidth(), (int)sizeOfWorld.getHeight(), background));
	}

	private static void addPlayer() {
		entities.addEntity((player = new Player((int) playerXY.getX(), (int) playerXY.getY(), 50, 50, sizeOfWorld)));
	}

	private static void addExit() {
		entities.addEntity((exit = new ExitBlock((int) exitXY.getX(), (int) exitXY.getY(), 50, 50)));
	}

	private static void addBlocks() {
		for (int i = 0; i < blockPosition.size(); i++) {
			entities.addEntity(new Block(blockSlidable.get(i), (int) blockPosition.get(i).getX(),
	...
	private static void addEnemies() {
		for (int i = 0; i < enemyPosition.size(); i++) {
			entities.addEntity(new Enemy((int) enemyPosition.get(i).getX(), (int) enemyPosition.get(i).getY(), 50, 50));
		}
	}
