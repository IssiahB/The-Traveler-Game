...
public class MainMenu extends MenuTemplate implements Menu {
	...
	private Rectangle play = new Rectangle(Loop.getWidth() / 2 - 100,
			Loop.getHeight() / 2 - 150, 200, 50);
	private Rectangle createWorld = new Rectangle(
			Loop.getWidth() / 2 - 100, Loop.getHeight() / 2 - 50, 200,
			50);
	private Rectangle settings = new Rectangle(
			Loop.getWidth() / 2 - 100, Loop.getHeight() / 2 + 50, 200,
			50);
	private Rectangle exit = new Rectangle(Loop.getWidth() / 2 - 100,
			Loop.getHeight() / 2 + 150, 200, 50);
	...
	public void update(int x, int y, boolean pressed,
		...
		} else if (inRect(x, y, createWorld) && released) {
		...
		} else if (inRect(x, y, settings) && released) {
			setColor = Color.GRAY;
	...
	public void render(Graphics g) {
		...
		g.drawString(Loop.getTitle(), Loop.getWidth() / 3 - 70, 50);
		...
		g.setFont(font);
		g.setColor(Color.white);
		g.drawString("Play", (int) (play.getX() + 55),
				(int) (play.getY() + 35));
		g.drawString("Create", (int) (createWorld.getX() + 40),
				(int) (createWorld.getY() + 35));
		g.drawString("Settings", (int) (settings.getX() + 25),
				(int) (settings.getY() + 35));
		g.drawString("Exit", (int) (exit.getX() + 55),
				(int) (exit.getY() + 35));
	}
}
